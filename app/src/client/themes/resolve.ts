...
function blendHex(fgHex: string, bgHex: string): string {
...
function nudgeBg(bgHex: string, isDark: boolean): string {
...
function rgb(hex: string | null | undefined, fallback: string): string {
  return hex ? hexToRgb(hex) : fallback
}

function toAlphaColor(rgbStr: string, alpha: number): string {
...
function isOpaqueHex(hex: string): boolean {
  const h = hex.replace(/^#/, '')
  if (h.length === 8) {
